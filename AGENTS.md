...
**GitHub Environment variables**: `AZURE_CLIENT_ID`, `AZURE_TENANT_ID`, `AZURE_SUBSCRIPTION_ID`, `AZURE_RESOURCE_GROUP`, `AZURE_CONTAINER_REGISTRY`, optional `APP_CUSTOM_DOMAIN`, `API_CUSTOM_DOMAIN`, `MARKETING_CUSTOM_DOMAIN`, `PUBLIC_WHATSAPP_PHONE`, `PUBLIC_GA_MEASUREMENT_ID`, `PUBLIC_META_PIXEL_ID`, `POSTGRES_LOCATION`, `POSTGRES_VERSION`, `OAUTH_*_CLIENT_ID`, `OAUTH_GOOGLE_SHEETS_CLIENT_ID`, `OAUTH_GOOGLE_ANALYTICS_CLIENT_ID`, `OAUTH_GOOGLE_BUSINESS_CLIENT_ID`, `OAUTH_GOOGLE_WORKSPACE_CLIENT_ID`, `OAUTH_META_FACEBOOK_CLIENT_ID`, `OAUTH_META_INSTAGRAM_CLIENT_ID`, `OAUTH_META_WHATSAPP_CLIENT_ID`, `META_WHATSAPP_EMBEDDED_SIGNUP_CONFIG_ID`, `SYSTEM_MAIL_PROVIDER`, `MAIL_FROM`, `AUDIT_MAIL_PROVIDER`, `AUDIT_MAIL_FROM`, `AUDIT_SMTP_USER`, and `SIGNUP_ADMIN_EMAIL`.
...
**GitHub Environment secrets**: `POSTGRES_ADMIN_PASSWORD`, `JWT_SECRET`, optional `OAUTH_*_CLIENT_SECRET`, `OAUTH_GOOGLE_SHEETS_CLIENT_SECRET`, `OAUTH_GOOGLE_ANALYTICS_CLIENT_SECRET`, `OAUTH_GOOGLE_BUSINESS_CLIENT_SECRET`, `OAUTH_GOOGLE_WORKSPACE_CLIENT_SECRET`, `OAUTH_META_FACEBOOK_CLIENT_SECRET`, `OAUTH_META_INSTAGRAM_CLIENT_SECRET`, `OAUTH_META_WHATSAPP_CLIENT_SECRET`, `META_APP_SECRET`, `META_WEBHOOK_VERIFY_TOKEN`, `CONNECTION_CREDENTIALS_KEY`, `RESEND_API_KEY`, `AUDIT_SMTP_PASS`.
...
- Marketing image bakes `PUBLIC_SITE_URL` / `PUBLIC_API_URL` / `PUBLIC_APP_URL` / `PUBLIC_APP_SIGNUP_URL` (and optional WhatsApp / analytics IDs) at Docker build time from the environment hostnames.
